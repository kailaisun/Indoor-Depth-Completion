...
def run_one_image(img, model):
    ...
    pred_np = np.array(y[0,:,:,3])*60000
    pred_np = np.uint16(pred_np)
    pred_np = np.where(pred_np > 0, pred_np, 0)
    # gtzero = (gt_np == 0.0)
    # pred_np[gtzero] = 0.0
    # gt_np[gtzero] = 0.0
...
img_np = np.load("/mnt/yangzhou/dataset/matterport/npy/test_full/pa4otMbVnkk/resize_b13bc7ca024046de92640c76f26a9f07_i1_3.npy")

# img_np = np.load(frame_path)
depth = img_np[:,:,3]
# depth = Image.fromarray(depth)
# depth = depth.resize((224,224),Image.NEAREST)
depth_ = cv2.resize(depth, (224,224), interpolation = cv2.INTER_AREA)
tg = img_np[:,:,4]
# tg = Image.fromarray(tg)
# tg = tg.resize((224,224),Image.NEAREST)
tg_ = cv2.resize(tg, (224,224), interpolation = cv2.INTER_AREA)

color = Image.fromarray(np.uint8((img_np[:,:,:-1])*255))
color_ = color.resize((224,224),Image.NEAREST)
color_ = np.array(color_)
        
img_np =  np.concatenate((color_/255,np.expand_dims(depth_,axis=-1),np.expand_dims(tg_,axis=-1)),axis=-1)

pred_np = run_one_image(img_np, model_mae)
pred = cv2.resize(pred_np, (320,256), interpolation = cv2.INTER_AREA)

def times60000(depth_nor):
    out = np.array(depth_nor)*60000
    out = np.uint16(out)
    out = np.where(out > 0, out, 0)
    return out
depth = times60000(depth)
tg = times60000(tg)

rgb_p = '/home/yangzhou/mae_color/rgb.jpg'
pred_p='/home/yangzhou/mae_color/pred.png'
# rgb = np.uint8((img_temp[:,:,:-2])*255)
...
def rgbd2pcd(color_p,depth_p,name):
    ...
    # pcd, ind = pcd.remove_radius_outlier(nb_points=16, radius=0.05)
    o3d.io.write_point_cloud(name,pcd)
rgbd2pcd(rgb_p,pred_p,pred_p[:-4]+'_n.ply')

# import scipy.io as sio
# save_path = '/home/yangzhou/bigroom.mat'
# sio.savemat(save_path, {'raw': depth,'pred':pred,'gt':tg})
