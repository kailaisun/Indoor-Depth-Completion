...
def run_one_image(img, model, imggt):
    ...
    gt_np = np.array(gt[0,:,:,3])*60000/4000
    pred_np = np.array(im_paste[0,:,:,3])*60000/4000
    # gtzero = (gt_np == 0.0)
    # pred_np[gtzero] = 0.0
    # gt_np[gtzero] = 0.0
    ...
    if ss<0.6:
        rgb2file(np.uint8((np.array(img[:,:,:-1]))*255),f'debug/{ss}_color.jpg')
...
test_path='/mnt/yangzhou/dataset/matterport/npy/test/'
...
for i,scene in enumerate(test_list):
    ...
    for j,frame in enumerate(frames):
        ...
        img_np =  np.concatenate((color/255,np.expand_dims(depth,axis=-1)),axis=-1)

        tg_path = os.path.join(gt_path,os.path.join(scene,frame[:-8]+'d'+frame[-7:-4]+'_mesh_depth.png'))
        tg_png = Image.open(tg_path)
        tg_png = tg_png.resize((224,224),Image.NEAREST)
        tg_png = np.array(tg_png) / 60000

        rmse_, mae_, ssip_ = run_one_image(img_np, model_mae,tg_png)
        rmse.append(rmse_)
        mae.append(mae_)
        ssip.append(ssip_)
        if ssip_ < 0.6:
